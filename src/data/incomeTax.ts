...
function marginalBand(income: number, bands: IncomeTaxBand[]): number {
  let lower = 0;
  for (const b of bands) {
    const upper = b.upTo ?? Infinity;
    if (income > lower && income <= upper) return b.rate;
    lower = upper;
  }
  return bands[bands.length - 1]?.rate ?? 0;
}

/** Marginal rate on the next dollar: the income-tax band rate plus the country's
 *  levy on that dollar. Uses a country `marginalRate` hook (UK NI + taper, IN
 *  cess) when a flat levy can't represent it, else falls back to flatLevyRate. */
...
export const INCOME_TAX_SCHEMES: Record<string, IncomeTaxScheme> = {
  AU: {
    ...
    offsets: ({ taxable }) => {
      ...
      else if (taxable <= 45000) lito = round(700 - (taxable - 37500) * 0.05);
      else if (taxable <= 66667) lito = round(325 - (taxable - 45000) * 0.015);
  ...
  GB: {
    ...
    flatLevyRate: 0,
    marginalRate: ({ gross, bandRate }) => {
      // In the £100k–£125,140 allowance-taper zone, each extra £1 also strips 50p
      // of allowance (taxed at the band rate) → effectively 1.5× the band rate.
      const incomeTaxMarginal = gross > 100000 && gross <= 125140 ? bandRate * 1.5 : bandRate;
      // Employee Class 1 NI on the next pound: 8% £12,570–£50,270, 2% above, 0 below.
      const niMarginal = gross > 50270 ? 0.02 : gross > 12570 ? 0.08 : 0;
      return incomeTaxMarginal + niMarginal;
    },
    levies: ({ gross }) => {
      ...
      return ni > 0 ? [{ name: 'National Insurance', amount: round(ni) }] : [];
    },
    offsets: () => [],
  ...
  IN: {
    ...
    marginalRate: ({ bandRate }) => bandRate * 1.04,
    offsets: ({ taxable, incomeTax }) => {
      ...
      return rebate > 0 ? [{ name: 'Section 87A rebate', amount: round(rebate) }] : [];
    },
    levies: ({ taxable, baseTax }) => {
      const surcharge = round(inSurcharge(taxable, baseTax));
      const cess = round((baseTax + surcharge) * 0.04);
  ...
  US: {
    ...
    flatLevyRate: 0,
    marginalRate: ({ gross, bandRate }) => {
      // FICA on the next dollar: Social Security 6.2% up to the $184,500 wage base,
      // Medicare 1.45% always, +0.9% additional Medicare above $200,000 (single).
      const socialSecurity = gross < 184500 ? 0.062 : 0;
      const medicare = 0.0145 + (gross > 200000 ? 0.009 : 0);
      return bandRate + socialSecurity + medicare;
    },
    levies: ({ gross }) => {
      ...
      const socialSecurity = round(Math.min(gross, 184500) * 0.062);
      const medicare = round(gross * 0.0145) + round(Math.max(0, gross - 200000) * 0.009);
      const items: IncomeLineItem[] = [];
...
export function calcIncomeTax(countryCode: string, gross: number, taxYear?: string): IncomeTaxResult | null {
  ...
  const incomeTax = round(progressive(taxable, set.bands));
