...
Depreciation regimes — New Zealand, the United Kingdom, Canada, Singapore, Ireland and
South Africa. Additive: every new `DepreciationRules`
...
- `DepreciationRules.regime` — `'effective_life' | 'rate_per_asset' | 'pooled_allowance' |
  'class_cca' | 'write_off_elective' | 'straight_line_fixed' | 'write_off_period' |
  'generic'`. The countries do not share a model, and a host branches on this
  ...
  Canada `class_cca`, Singapore `write_off_elective`, Ireland `straight_line_fixed`,
  South Africa `write_off_period`, the fallback `generic`.
