...
- `DepreciationRules.landlordSmallItemDeduction?(onDate)` — optional, returning `LandlordSmallItemInfo` (`InstantAssetWriteOffInfo` plus an optional `pool`). The per-item rule for a landlord who is **not** carrying on a business, which differs from the business `instantAssetWriteOff` in every country surveyed. Absent (the generic rules, IE) means "not published". A host acts only on `verified: true` with a non-null `limit`; everything else prints the note and defaults nothing.
